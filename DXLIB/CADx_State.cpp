...
namespace DX
{
    ...
    ID3D11DepthStencilState* CADx_State::m_pDSSDepth_Less_Equal = 0;
...
#pragma region DEPTH_STENCIL
        ...
        if (FAILED(hr = pDevice->CreateDepthStencilState(&dsDesc, &m_pDSSDepth_Less_Equal)))
        {
...
#pragma endregion   DEPTH_STENCIL
        ...
        Set_RSState(CADevice::m_pImmediate_Device_Context, m_pRSSolid_Frame);
        Set_BState(CADevice::m_pImmediate_Device_Context, m_pAlpha_Blend);
        Set_DSState(CADevice::m_pImmediate_Device_Context, m_pDSSDepth_Less_Equal);
        Set_SState(CADevice::m_pImmediate_Device_Context, m_pSSWrap_Linear);
    }
