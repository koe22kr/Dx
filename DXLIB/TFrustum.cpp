...
namespace DX
{
    bool T_PLANE::CreatePlane(D3DXVECTOR3 v0, D3DXVECTOR3 v1, D3DXVECTOR3 v2)
    {
    ...
    bool T_PLANE::CreatePlane(D3DXVECTOR3 vNormal, D3DXVECTOR3 v0)
    {
        D3DXVec3Normalize(&vNormal, &vNormal);
    ...
    void TFrustum::ExtractPlanesD3D(T_PLANE* pPlanes, const D3DXMATRIX & comboMatrix)
    {
    ...
    bool TFrustum::CreateRenderBox(ID3D11Device*  pd3dDevice, ID3D11DeviceContext* pImmediateContext)
    {
        ...
        SAFE_NEW(m_pBoxShape, CADx_Shape_Box);
        if (FAILED(m_pBoxShape->Create(pd3dDevice,L"PS" ,L"../../data/shader/box.hlsl")))
        {
            MessageBox(0, _T("m_LineShape 실패"), _T("Fatal error"), MB_OK);
            return false;
        }
        // vertex color
        m_pPixelShader.Attach(DX::LoadPixelShaderFile(pd3dDevice, L"../../data/shader/box.hlsl"));
    ...
    bool TFrustum::PreRender(ID3D11DeviceContext*    pContext)
    {
        ...
        pContext->UpdateSubresource(m_pBoxShape->m_helper.m_pVertexBuffer.Get(), 0, NULL, &m_VertexList.at(0), 0, 0);
        m_pBoxShape->SetMatrix(NULL, &m_mView, &m_mProj);
        m_pBoxShape->m_helper.PreRender(pContext);
        return true;
    }
    ...
    {
        pContext->UpdateSubresource(m_pBoxShape->m_helper.m_pConstantBuffer.Get(), 0, NULL, &m_pBoxShape->m_cbData, 0, 0);
        m_pBoxShape->m_helper.PostRender(pContext);
        return true;
    }
    ...
    BOOL TFrustum::ClassifyPoint(D3DXVECTOR3* pv)
    {
        float		fPlaneToPoint;
    ...
    // 구 제외 처리
    BOOL TFrustum::ClassifySphere(T_SPHERE* pSphere)//D3DXVECTOR3* pv, float radius )
    {
